/** Full set of Google Places Table A types across all 19 categories.
 *  Used to generate synthetic visits with random place types. */
export const TABLE_A_PLACE_TYPES: string[] = [
    // Automotive
  ...
  /**
   * The generated lookup dictionary: { "car_repair": "Automotive", ... }
   */
  export const placeTypeLookup: Record<string, string> = Object.entries(place_categories).reduce(
    (acc, [category, types]) => {
      types.forEach((type) => {
        acc[type] = category;
      });
  ...
  /**
 * Helper function to safely get the category
 */
export const getCategory = (type: string): string => {
  return placeTypeLookup[type] ?? "Other";
};